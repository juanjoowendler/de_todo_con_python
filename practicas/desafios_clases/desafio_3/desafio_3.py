...
vector = vector_known_range(300000)
...
"""
diferentes = [0] * len(vector)
cant_diferentes = 0
for i in range(len(vector)):
    elemento = vector[i]
    diferentes.append(elemento)
    if elemento in diferentes:
        cant_diferentes += 1

print("Cantidad de elementos diferentes: ", cant_diferentes)
"""
...
for i in vector:
    num = vector[i]
    cont[i] += 1
    for elemento_x in vector:
        shell_sort(vector)
        if binary_search(vector, num) == -1:
            num.append(num)
            cont.append(1)
        else:
            cont += 1
...
print("Cantidad de elementos de vector: ", len(vector))
mayor = max(vector)
menor = min(vector)
print("Mayor = ", mayor, end=" | ")
print("Menor = ", menor)
"""
for i in range(len(vector)):
    num[i] = vector[i]
...
print("Tamaño final del arreglo num[] = ", num)
"""


for numero_x in vector:
    c[numero_x] += 1

"""
for numero_x in vector:
    c[numero_x] += 1
    i = binary_search(num, numero_x)
    if i != -1:
        cont[i] += 1
    else:
        num.append(numero_x)
...
print("TAMAÑO FINAL DEL ARREGLO num = ", len(num))
...
print(Counter(vector))
print("Cantidad de casilleros diferentes de 0 en c = ", cantidad)
